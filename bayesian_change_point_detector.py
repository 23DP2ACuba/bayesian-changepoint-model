...
class BayesianChangePointModel:
    def __init__(self, data, lambda_rate=1.0, max_num_changepoints=MAX_NUM_CHANGEPOINTS):
        self.data = tf.convert_to_tensor(data['Close'].values, dtype=tf.float32)
        self.num_points = len(data)
        self.poisson_prior = tfdist.Poisson(rate=lambda_rate)
    ...
    def joint_log_prob(self, changepoint_locs, means, stddevs):
        ...
        rv_means = tfdist.Normal(loc=0., scale=20.)
        rv_stddevs = tfdist.HalfNormal(scale=0.5)
        
        # Convert changepoint locations to integers and add boundaries
        ...
        # Compute segment likelihoods using tf.map_fn
        def compute_segment_likelihood(i):
            return
        
        segment_indices = tf.range(tf.size(all_cps) - 1)
        log_likelihood = tf.reduce_sum(
            tf.map_fn(compute_segment_likelihood, segment_indices, dtype=tf.float32)
        )
